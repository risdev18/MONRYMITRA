...
export default function CustomerDetails() {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto bg-[#F7F9FC] min-h-screen">
            <button onClick={() => navigate(-1)} className="group flex items-center text-slate-500 mb-8 hover:text-emerald-600 transition font-black text-sm uppercase tracking-widest">
                <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition" /> Back
            </button>

            {/* Header Card */}
            <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap justify-between items-center mb-8 gap-6">
                <div className="flex items-center gap-6">
                    <div className="w-20 h-20 rounded-3xl bg-emerald-50 flex items-center justify-center text-3xl font-black text-emerald-600 shadow-inner border border-emerald-100">
                        {customer.name[0]}
                    </div>
                    <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                <button
                    ...
                    className="flex items-center justify-center gap-3 bg-emerald-600 text-white py-5 rounded-2xl hover:bg-emerald-700 font-black transition shadow-lg shadow-emerald-100 active:scale-95"
                >
                    <MessageCircle className="w-5 h-5" /> Send Reminder (WhatsApp)
                </button>
                <button
                    onClick={() => setShowPayModal(true)}
                    className="flex items-center justify-center gap-3 bg-slate-900 text-white py-5 rounded-2xl hover:bg-slate-800 font-black transition shadow-lg shadow-slate-200 active:scale-95"
                >
            ...
            <div className="flex gap-4 p-2 bg-slate-100 rounded-2xl mb-8 w-fit">
                {['overview', 'history', 'attendance'].map(tab => (
                    <button
                        ...
                        className={`px-8 py-3 font-black text-xs uppercase tracking-widest rounded-xl transition ${activeTab === tab ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                        {tab}
                    </button>
                ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                            <h3 className="font-black text-slate-800 mb-6 flex items-center gap-3 text-sm uppercase tracking-wider">
                                <Clock className="w-5 h-5 text-emerald-500" /> Plan & Schedule
                            </h3>
                                    ...
                                    <tr key={tx._id} className="hover:bg-slate-50/50 transition">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <button
                                                ...
                                                className="p-3 bg-slate-50 text-emerald-600 rounded-xl hover:bg-emerald-50 transition shadow-sm"
                                            >
                    ...
                    <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm">
                        <div className="flex flex-wrap justify-between items-center mb-8 gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 text-emerald-600 font-black text-xs uppercase tracking-widest hover:bg-emerald-50 px-4 py-2 rounded-xl transition border border-emerald-100"
                            >
...
function PaymentModal({ onClose, onSubmit, customerName, currentDue }: any) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl shadow-slate-900/20 border border-slate-50">
                ...
                <div className="space-y-8">
                    <div>
                        <div className="flex justify-between items-center mb-2 px-1">
                            ...
                            <button
                                ...
                                className="text-[10px] font-black text-emerald-600 hover:text-emerald-800 uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-full transition"
                            >
                                Pay Full: ₹{currentDue}
                            </button>
                        </div>
                        <div className="relative">
                            ...
                            <input
                                type="number"
                                className="w-full pl-12 pr-6 py-5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-black text-slate-800 text-2xl transition"
                                value={amount}
                    ...
                    <div>
                        ...
                        <input
                            type="date"
                            className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-black text-slate-900"
                            value={nextDate}
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {['CASH', 'UPI', 'GPAY'].map(m => (
                                <button
                                    key={m}
                                    onClick={() => setMode(m)}
                                    className={`py-4 rounded-xl text-xs font-black transition-all border-2 ${mode === m ? 'bg-emerald-50 border-emerald-600 text-emerald-700 shadow-md' : 'bg-white border-slate-100 text-slate-400 hover:bg-slate-50'}`}
                                >
                                    {m}
                                </button>
                            ))}
                ...
                <div className="flex flex-col gap-3 mt-10">
                    <button
                        ...
                        className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
